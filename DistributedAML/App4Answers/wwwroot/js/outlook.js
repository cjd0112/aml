...
$(function() {
    ...
    var dataManger1 = ej.DataManager({
        url: window["baseurl"] + "api/Mail/SubscriptionInfoFlat",
        crossDomain: true
    });
    ...
    dataManger1.executeQuery(ej.Query()).done(function(e) {
        ...
        */
        ...
        $("#treeView").ejTreeView({
            fields: { dataSource: e.result.Subscriptions }
        });
        ...
        $("#rteSample").ejRTE({
            ...
            height: "350px",
            toolsList: [
                "formatStyle", "font", "style", "effects", "alignment", "lists", "indenting", "clipboard", "doAction",
                "clear", "casing", "print"
            ]
        });
    ...
    function onMouseDown(args) {
        ...
        data = $($(".e-treeview").find(".e-active").closest("ul").parent()).next().length !== 0
            ? $($(".e-treeview").find(".e-active").closest("ul").parent()).next()
            : $($(".e-treeview").find(".e-active").closest("ul").parent()).prev();
        if (args.item.closest("#templatelist") && args.item.find(".subjectstyle").attr("style") != "undefined") {
            if (selectnode == -1 &&
            (args.item.find(".subjectstyle").css("font-weight") == "bold" ||
                args.item.find(".subjectstyle").css("font-weight") == "700")) {
            ...
            (args.item.find(".subjectstyle").css("font-weight") == "bold" ||
                ...
                for (var j = 0; j < data.find("li").length; j++) {
                    ...
                    if ($(".e-treeview").find(".e-active").find(".treeitem").text() ==
                        $(data.find("li")[j]).find(".treeitem").text()) {
        ...
        $(args.item).find(".designationstyle")
            .css({ "font-weight": "normal", "font-family": "Helvetica, Segoe UI", color: "#333" });
        $(args.item).find(".subjectstyle")
            .css({ "font-weight": "normal", "font-family": "Helvetica, Segoe UI", color: "#333" });
...
//treenode click
    ...
    function updateReadUnread(read, readcount, args) {
        ...
        if (!ej.isNullOrUndefined(args.currentElement.nextSibling))
            args.currentElement.nextSibling.textContent = readcount;
        setCount(args);
    ...
    function setCount(args) {
        var value = $(args.currentElement).closest("li").parent().parent().next().length !== 0
            ? $(args.currentElement).closest("li").parent().parent().next().find("ul")
            : $(args.currentElement).closest("li").parent().parent().prev().find("ul");
        for (var i = 0; i < value.children().length; i++) {
            if (args.currentElement.textContent == $(value.children()[i]).find(".treeitem").text()) {
                $(value.children()[i]).find("#count").text((args.currentElement.textContent == "Drafts")
                    ? readcountd
                    : (args.currentElement.textContent == "Inbox")
                    ? readcounti
                    : readcountc);
            }
        }
    ...
    function Click(args) {
        ...
        var title = $("#mailsubject").ejMaskEdit("option", "value") == null
            ? "(No Subject)"
            : $("#mailsubject").ejMaskEdit("option", "value");
        var msg = $("#rteSample").ejRTE("option", "value") == ""
